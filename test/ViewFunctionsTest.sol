...
contract ViewFunctionsTest is BaseTest {
    ...
    function testConvertToAssets(uint256 initialDeposit, uint256 interest, uint256 shares) public {
        ...
        _writeTotalAssets(initialDeposit + interest);
    ...
    function _setupForFeeViews(TestData memory data) internal returns (uint256, uint256) {
        ...
        strategy.setInterest(data.interest);
        skip(data.elapsed);
    ...
    function testPreviewDeposit(TestData memory data, uint256 assets) public {
        (uint256 newTotalAssets, uint256 newTotalSupply) = _setupForFeeViews(data);
        assets = bound(assets, 0, maxTestAssets);
