...
contract WithdrawalQueueTest is BaseTest {
    ...
    /// @notice Partial cancel: caller picks how much to cancel, shares restored pro-rata.
    function testPartialCancelRestoresProRataShares() public {
        uint256 deposit = 1_000e18;
        uint256 sharesIssued = _seed(deposit, deposit);

        // Queue 600 pending (in two requests to verify the slot is accumulated).
        vm.startPrank(alice);
        vault.withdraw(200e18, alice, alice);
        vault.withdraw(400e18, alice, alice);
        vm.stopPrank();

        (uint128 pendingAssetsBefore, uint128 pendingSharesBefore) = vault.pendingWithdrawal(alice);
        assertEq(uint256(pendingAssetsBefore), 600e18);
        assertEq(uint256(pendingSharesBefore), 600e18, "1:1 burn ratio at fresh prices");

        // Cancel 300 of 600. Pro-rata: 300 × 600 / 600 = 300 shares restored.
        uint256 expectedRestore = 300e18;
        uint256 aliceSharesBefore = vault.balanceOf(alice);

        vm.expectEmit(true, false, false, true);
        emit EventsLib.WithdrawalCancelled(alice, expectedRestore, 300e18);
        vm.prank(alice);
        uint256 restored = vault.cancelWithdrawal(300e18);

        assertEq(restored, expectedRestore, "pro-rata share count");
        assertEq(vault.balanceOf(alice) - aliceSharesBefore, expectedRestore, "shares re-minted");

        // Remaining pending halved.
        (uint128 pa, uint128 ps) = vault.pendingWithdrawal(alice);
        assertEq(uint256(pa), 300e18, "remaining assets");
        assertEq(uint256(ps), 300e18, "remaining shares");
        assertEq(vault.pendingClaimableAssets(), 300e18, "earmark reduced");

        // The remaining 300 can be cancelled or claimed independently later.
        sharesIssued; // silence
    }

    function testPartialCancelExactFullAmountClearsSlot() public {
        uint256 deposit = 1_000e18;
        _seed(deposit, deposit);
        ...
        vm.prank(alice);
        vault.cancelWithdrawal(400e18); // exactly the pending amount

        (uint128 pa,) = vault.pendingWithdrawal(alice);
        assertEq(uint256(pa), 0, "slot fully cleared");
        assertEq(vault.pendingClaimableAssets(), 0);
    }

    function testPartialCancelRejectsZero() public {
        uint256 deposit = 1_000e18;
        _seed(deposit, deposit);
    ...
    function testPartialCancelRejectsExcess() public {
        uint256 deposit = 1_000e18;
        _seed(deposit, deposit);
        ...
        // Anything > pending must revert.
        vm.expectRevert(ErrorsLib.InvalidRequest.selector);
        vm.prank(alice);
        vault.cancelWithdrawal(400e18 + 1);
    }

    function testPartialCancelRejectsWithoutPending() public {
        // No prior withdraw — pending is zero.
        vm.expectRevert(ErrorsLib.RequestNotPending.selector);
        vm.prank(alice);
        vault.cancelWithdrawal(100e18);
    }
